...
export default function ExperimentBlog({ onBack }) {
  const auditContent = {
    title: "Informe de Auditoría de Rendimiento: Navegadores Web y Ecosistema del Sistema",
    date: "8 de Mayo de 2026",
    analyst: "Vladivostok (Ingeniero de Sonido) & Prompt Master (Gemini)",
    script: `param (  
    [string]$TargetName = "",  
    [int]$TargetId = 0  
)
    ...
    >
      ...
      <header className="w-full pt-[max(1.5rem,env(safe-area-inset-top))] px-6 sm:px-12 pb-6 border-b border-white/5 bg-black/80 backdrop-blur-xl z-20 flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xs font-black uppercase tracking-[0.3em] text-[#39FF14]">Vostok_Log_Entry</h1>
            <p className="text-[10px] font-mono text-slate-500 uppercase tracking-widest">{auditContent.date}</p>
          </div>
        </div>
        <div className="hidden md:flex items-center gap-6">
          <div className="flex flex-col items-end">
            <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest">Estado</span>
            <span className="text-[10px] font-mono text-cyan-400 uppercase font-black tracking-widest">Publicado_Verificado</span>
          </div>
          <div className="w-px h-8 bg-white/10" />
          <div className="flex flex-col items-end">
            <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest">Protocolo</span>
            <span className="text-[10px] font-mono text-slate-400 uppercase font-bold tracking-widest">Audit_V2.0</span>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar z-10">
        <article className="max-w-4xl mx-auto px-6 py-16 sm:px-12">
          {/* Post Intro */}
          <div className="mb-20">
            <motion.div 
              ...
              className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#39FF14]/10 border border-[#39FF14]/20 text-[#39FF14] text-[9px] font-black uppercase tracking-[0.3em] mb-8"
            >
            ...
            <motion.h2 
              ...
              className="text-4xl md:text-6xl font-black tracking-tighter uppercase leading-[0.9] mb-8"
            >
              {auditContent.title}
            </motion.h2>
            <motion.div 
              ...
              className="flex flex-wrap gap-8 text-slate-500"
            >
              <div className="flex flex-col">
                <span className="text-[8px] font-black uppercase tracking-widest mb-1 text-slate-700">Analista</span>
                <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{auditContent.analyst}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[8px] font-black uppercase tracking-widest mb-1 text-slate-700">Tiempo de Lectura</span>
                <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">6 MIN</span>
              </div>
            </motion.div>
          </div>
            ...
            <section className="space-y-6">
              ...
              <div className="p-6 bg-cyan-950/20 border border-cyan-500/20 rounded-2xl">
                <div className="flex gap-4">
                  ...
                  <p className="text-sm italic">"Este script es fundamental para auditar el impacto de procesos externos mientras el DAW (Reaper/Ableton) está en sesión."</p>
                </div>
              </div>
            </section>
            ...
            <section className="space-y-6 pb-20">
              ...
              <ul className="space-y-4">
                {[
                  ...
                  { title: "Manejo de Ecosistemas Pesados", desc: "Cerrar procesos en segundo plano de suites creativas (como Adobe) antes de sesiones de alta carga computacional para evitar picos de red y CPU." },
      ...
      <footer className="w-full py-8 px-6 sm:px-12 border-t border-white/5 bg-black z-20 flex justify-between items-center">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-mono text-slate-500 uppercase tracking-widest">End_Of_File</span>
        </div>
        <button 
          onClick={onBack}
